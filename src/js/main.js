...
var $ = require('jquery'),
    ...
    LayerAttributesPanel = require('./views/LayerAttributesPanel'),
    Canvas = require('./views/Canvas'),
    LayerList = require('./views/LayerList'),
...
document.getElementById('data').addEventListener('click', function (event) {
    ...
    if (lay.length > 0) {
        ...
        lay.forEach(function (cid) {
            var layer = layerList.layers.get({ cid: cid });
            if (layer && !layerList.selectedLayers.get({ cid: cid })) {
