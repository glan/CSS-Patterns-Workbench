 ...
 */
function Incrementable(textField, multiplier, units) {
    var me = this;

    this.textField = textField;

    this.step = +textField.getAttribute('step') || +textField.getAttribute(
        'data-step') || 1;
    ...
    this.textField.addEventListener('keydown', function (evt) {
        ...
        if (multiplier && (evt.keyCode == 38 || evt.keyCode == 40)) {
            ...
            var caret = this.selectionStart,
                text = this.value,
                regex = new RegExp('^([\\s\\S]{0,' + caret +
                    '}[^-0-9\\.])(-?[0-9]*(?:\\.?[0-9]+)(?:' + me.units +
                    '))\\b', 'i'),
                property = 'value' in this ? 'value' : 'textContent';

            this[property] = this[property].replace(regex, function ($0, $1,
                $2) {
                if ($1.length <= caret && $1.length + $2.length >=
                    caret) {
                    me.changed = true;
                    return $1 + me.stepValue($2, evt.keyCode == 40,
                        multiplier);
                } else {
                    return $1 + $2;
                }
    ...
    this.textField.addEventListener('keypress', function (evt) {
        if (me.changed && me.checkModifiers(evt) && (evt.keyCode == 38 ||
            evt.keyCode == 40))
            evt.preventDefault();
        evt.stopPropagation();
        me.changed = false;
...
var proto = Incrementable.prototype;

/**
 * Gets a <length> and increments or decrements it
 */
proto.stepValue = function stepValue(length, decrement, multiplier) {
...
proto.units = '|%|deg|px|r?em|ex|ch|in|cm|mm|pt|pc|vmin|vw|vh|gd|m?s';

module.exports = Incrementable;
